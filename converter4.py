def converter4(fileName): 
	...
	with open(fileName, mode = 'r') as CRISPR_data_new: 
		forgot = []
		forgot_recon = []
		new_lines = []
		k = 0
	...
	for i in range(len(forgot)): 
		forgot[i] = forgot[i] + '_AT1'
		if forgot[i] in model.genes: 
			forgot_recon.append(forgot[i])
	
				
	#choosing a relevant name for the output file 
	assayName =  fileName[0:-4]
	...
	return open(OutputFileName, 'r'), forgot_recon
	
converter4('temp.txt')

'''
Out of the 19 007 NALM6 genes that were KO w/ CRISPR

All converted to Entrez ID !! 

1887 in Recon3

#make model closer to NALM6 by removing unexpressed genes
#double KOs 
#litt review of model
#MOMA? 
