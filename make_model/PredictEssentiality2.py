...
def PredictEss(FileName, OutFileName):
    ...
    with open(FileName) as file: 
        ...
        for lines in file: 
            ...
            if count > 2 : # CHANGE IT DEPENDING ON FILE 
                if line_split[1] not in reducedExp: 
                    print(line_split[0])
                    print(line_split[1])
                    myList.append(line_split[0]) # CHANGE IT DEPENDING ON FILE
                    myList.append(line_split[1]) # CHANGE IT DEPENDING ON FILE
...
PredictEss('allSingleGeneDel_NALM31.txt', 'EssPred31.txt')
